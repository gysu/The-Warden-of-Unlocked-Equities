...
# ==========================================
# 0. Discord Webhook 文字推播模組 (支援長表格自動分段)
# ==========================================


def send_discord_table_message(webhook_url, content_text):
    """將文字/表格透過 Discord Webhook 發送，避免超過 2000 字元上限"""
    if not webhook_url:
        print('⚠️ 未設定 DISCORD_WEBHOOK_URL，跳過 Discord 發送。')
        return

    # 若總長度超過 1800 字，按列自動切段並補上 codeblock
    if len(content_text) <= 1900:
        ...
        for line in lines:
            if '市場' in line or '|:--' in line:
                header_lines.append(line)

            if len(curr_chunk) + len(line) + 10 > 1850:
                if curr_chunk.startswith('```text') and not curr_chunk.endswith(
                    '```'
                ):
                    curr_chunk += '\n```'
...
if results:
    ...
    # 產生 Markdown 表格
    table_str = tabulate(
        df_result, headers='keys', tablefmt='pipe', showindex=False
    )
